...
서비스 주소를 사용합니다. 팀 서버의 읽기 전용 DB·ML과 실제 계약서 분석을 모두 켜려면
...
`product`나 Git에 복사하지 않습니다. 계약서만 실제로 시험하고 DB는 Mock으로 유지하려면
`COMPANY_DATA_MODE=mock`으로 두면 됩니다.
...
`configured_unreachable`, 설정 자체가 없으면 `unavailable`로 표시됩니다.
...
가능하면 가상의 이름·회사로 만든 합성 계약서를 사용합니다. 자동 Mock 전환은 기본적으로 꺼져 있으므로
연결 실패가 실제 결과처럼 표시되지 않습니다.
