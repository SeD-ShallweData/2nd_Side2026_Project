...
export const KNOWN_ROUTING_GAPS: PolicyRoutingCase[] = [
  {
    ...
    expect_sources: "none",
    note:
      "임금 안내로 가는 것이 자연스러운 질문인데 '사업장' 키워드가 먼저 걸려 " +
      "회사 검색 안내로 빠진다. 회사 맥락 분기가 임금 분기보다 앞에 있기 때문이다.",
  },
  {
    id: "gap-02",
    branch: "company_reason",
  ...
  },
  {
    ...
    expect_sources: "none",
    note:
      "회사 안전을 묻는데 '회사 안전'이 아니라 '회사가 안전'이라 키워드가 빗나가, " +
      "사업장 선택을 안내하는 limited 분기 대신 일반 fallback(passed)으로 간다.",
  },
];
