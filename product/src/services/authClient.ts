...
function toAuthApiError(status: number, rawBody: string): AuthApiError {
  ...
  return new AuthApiError(status, code, message, retryable, requestId, readErrorDetails(envelope?.details));
}
...
async function requestAuthApi<T>(
  ...
  if (!response.ok) throw toAuthApiError(response.status, rawBody);
