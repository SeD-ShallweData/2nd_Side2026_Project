...
`completed`(처리완료)는 향후 상태 변경 주체가 확정됐을 때 사용한다. 감독관은 조회만 한다.

제출자의 내부 ID는 저장하지만 응답에는 이메일·내부 사용자 ID·원본 파일명·원본 사진 URL을 포함하지
않는다. 감독관에게 제공하는 사진 사본은 EXIF 위치·시각·기기정보와 XMP·IPTC를 제거한다.
...
| `GET /api/worksite-tips` | `inspector` | 미처리 상태 우선, 동일 상태 최신순 제보 목록 |
...
| 필드 | 필수 | 제한 |
| --- | --- | --- |
| `category` | 필수 | `wage`(임금) 또는 `safety`(산재) |
...
접수 화면은 다음 고지 문구를 반드시 표시한다.

> 본 제보함은 신고자의 익명성과 프라이버시를 철저히 보호하며, 입력된 정보는 공공 노동 정보 서비스의 확인 및 점검 참고용으로만 안전하게 활용됩니다.

접수 성공은 `201`이며 다음 값만 반환한다. 이 영수증을 제외한 일반 사용자의 제보 재조회 API는 현재
제공하지 않는다.

- `source`
- `tip_id`
- `category=wage|safety`
- `status=received`
- `title`
- `body`
- `submitted_at`
...
목록은 `page`와 `limit`을 지원한다. 기본값은 각각 1과 10이며 `limit`은 최대 20이다. 정렬 우선순위는
`received` → `in_progress` → `completed`이며, 동일 상태에서는 `submitted_at` 최신순이다. 목록에는 본문
전체가 아니라 최대 160자의 `body_preview`와 사진 개수만 포함한다.
...
`0010` migration이 `worksite_tips`와 `worksite_tip_attachments`를 만들고, `0013` migration이 K5의
임금·산재 분류와 처리 상태를 추가한다. 기존 고정 분류 `worksite_tip` 행은 산재 제보인 `safety`로
변환한다. DB에는 사진의 `storage_key`,
