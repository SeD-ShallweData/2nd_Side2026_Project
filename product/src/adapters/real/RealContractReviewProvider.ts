...
function toItem(value: unknown, index: number): ContractItem | null {
  ...
  return {
    ...
    legal_basis: string(finding.law),
...
export class RealContractReviewProvider implements ContractReviewProvider {
  ...
  async review(request: ContractReviewRequest): Promise<ContractReviewResult> {
    ...
    const payload = (await response.json().catch(() => ({}))) as CshReviewResponse;
    ...
    const items = Array.isArray(payload.verdict?.findings)
      ? payload.verdict.findings.map(toItem).filter((item): item is ContractItem => item !== null)
      : [];
    return {
      ...
      warnings: string(payload.verdict?.headline) ? [string(payload.verdict?.headline)!] : [],
      ...
      file_name: string(payload.filename) ?? request.file.name,
    };
  }
}
