...
6쌍의 M2·M4·M5·M6·M7을 뽑아 **정상 띠**를 정한다. 화면은 이 띠를 배경으로 그린다. 새 배치가 띠 밖이면 노란 경고. **이것이 ML 엔지니어가 아닌 멤버가 "이 배치 이상한데?"를 말할 수 있게 하는 장치다.**

#### 7.7.1 실측 결과 (2026-09-18 · Q12 종결)

> ⚠️ **초안이 예시로 든 「상위100 겹침 78~85%」는 실측과 방향이 반대다.** 실제는 **19~31%** 다.
> 상위 100곳 중 매달 70~80곳이 바뀐다. 이 수치를 전제로 화면을 설계할 것.

**전제 확인** `[실측 · 운영 DB SELECT 전용]`

| 항목 | 결과 |
| --- | --- |
| 모델 동일성 | 7배치 전부 `door1-voting-39f-v1` · `model_sha cbe5d951f170527c` → **M0 기준 6쌍 모두 「월간 변화」**. 모델 교체 오염 없음 |
| 적재 | 7배치 모두 `ingested_at = 2026-08-14 15:02:34`(Path B 정본). 한 번에 backfill 됐으나 **같은 모델을 7개 관측월에 각각 돌린 결과**라 쌍별 차이는 「관측창 1개월 이동 시 출력 변화」를 그대로 잰다 |

**쌍별 실측**

| 쌍 | M5 K=100 | K=500 | K=1500 | K=3000 | M6 상관 | M4 등급변경 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| **① 25-12→26-01** | **19.0%** | **31.6%** | **40.4%** | **46.6%** | **0.622** | **9.9%** |
| ② 26-01→26-02 | 23.0% | 37.6% | 47.5% | 55.3% | 0.738 | 8.3% |
| ③ 26-02→26-03 | 29.0% | 46.0% | 55.7% | 61.4% | 0.756 | 5.2% |
| ④ 26-03→26-04 | 25.0% | 42.0% | 53.1% | 61.8% | 0.704 | 6.7% |
| ⑤ 26-04→26-05 | 31.0% | 40.6% | 55.6% | 60.8% | 0.754 | 5.1% |
| ⑥ 26-05→26-06 | 23.0% | 40.4% | 51.0% | 57.0% | 0.723 | 5.8% |

배치 단위 지표(M2·M7)는 7개 값이다.

| 관측월 | M2 채점불가 | M7 안정신호 | 유보 | 유보_정보부족 | 배제_* |
| --- | ---: | ---: | ---: | ---: | ---: |
| **25-12** | 8.12% | 6.3% | **79.0%** | **10.2%** | 4.5% |
| 26-01 | 7.93% | 6.7% | 85.4% | 3.5% | 4.5% |
| 26-02 | 8.30% | 6.8% | 85.4% | 3.3% | 4.4% |
| 26-03 | 8.46% | 6.8% | 85.8% | 3.0% | 4.4% |
| 26-04 | 9.17% | 6.7% | 85.9% | 3.0% | 4.4% |
| 26-05 | 9.13% | 6.7% | 85.8% | 3.1% | 4.4% |
| 26-06 | 8.98% | 6.5% | 85.7% | 3.5% | 4.4% |

#### 7.7.2 🔴 첫 쌍(①)은 띠에서 뺀다

**① 이 6개 지표 전부에서 극단값이다** — M5 4종·M6 최저, M4 최고. 6/6 은 우연으로 보기 어렵다.

원인 방향은 확인됐다 `[실측]` — **초기 배치일수록 사업장 이력이 덜 쌓였다.**

```
n_months 10개월 이상    배치1 439,338  →  배치7 469,962   (3만 곳 증가)
유보_정보부족           배치1  51,991  →  나머지 ~15,000  (3배)
```

앞으로 들어올 배치는 이력이 충분하므로 ②~⑥ 을 닮는다. ① 을 넣으면 띠가 넓어져 **진짜 이상 배치를 놓친다.**

> 근본 원인은 단일 컬럼으로 특정되지 않았다 `[미확인]`. `n_months`(정상)·`risk_full IS NULL`(정보부족 행 중 0건)·`n_green`(4.01 vs 3.73~3.79)·`door1_ever`(차이 미미)를 확인했으며, 판정이 복합 규칙이라 조합 효과로 보인다. `wage-risk.md` 판정 규칙 대조가 남아 있다.

#### 7.7.3 화면에 그릴 정상 띠 — **① 제외 5쌍 기준**
...
표본이 5쌍뿐이라 통계 구간(평균±σ)이 아니라 **관측 최소~최대**다. 여유(버퍼)는 두지 않는다 — 오탐이 나면 그때 넓힌다. 좁게 시작해야 이상을 잡는다.

#### 7.7.4 화면에 반드시 함께 표기할 것

1. **「모델 고정 기준」** — 재학습 모델 배치에는 이 띠가 적용되지 않는다. 그때 다시 재야 한다
2. **표본 5쌍** — 통계적 신뢰구간이 아니라 관측 범위다
3. **첫 배치 제외** — 이력 미성숙으로 뺐다

#### 7.7.5 표현 주의 — 「자동으로 잡아낸다」는 과하다

띠는 **표시**일 뿐 자동 차단·롤백이 아니다. 사람이 보고 판단한다.
대외 문구는 *"직전 배치와 비교해 평소 범위를 벗어나면 표시합니다"* 수준으로 적는다.

> 산출 근거와 실행한 조회 목록: `.context/report/2026-09-18_13-33-46_산출-배치비교-기준선띠.md`

#### 7.7.6 사유 순위 기준 — M9 (2026-09-19 · Q13 종결)

M9(`reasons` 상위 10 + 순위 변화)는 위 띠와 **성격이 정반대**라 따로 적는다.
...
| --- | --- |
| 배치당 큐 행수 | 3,000 |
| 행당 사유 수 | **정확히 3.00** (7배치 전부) |
| 배치당 사유 개수 | 9,000 |
| 사유 **종류** | **16~19개** |

사유 종류가 스무 개 안쪽이라 **상위 10이 전체의 대부분을 덮는다.** 63.9만 중 3,000을 뽑는 큐와 성격이 다르다.

**배치별 상위 10 순위** `[실측]`

| 사유 | 25-12 | 26-01 | 26-02 | 26-03 | 26-04 | 26-05 | 26-06 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 체납액 | 1 | 1 | 1 | 1 | 1 | 1 | 1 |
| `door1_maxmonths` | **3** | 2 | 2 | 2 | 2 | 2 | 2 |
| 4대보험 체납이력 | **6** | 3 | 4 | 3 | 3 | 3 | 3 |
| 고지금액 변동 | **2** | 4 | 3 | 4 | 4 | 4 | 4 |
| `imputed_months_count` | 4 | 5 | 5 | 5 | 5 | 5 | 5 |
| `imputed_ratio` | 5 | 6 | 6 | 7 | 7 | 6 | 6 |
| 지역 | 7 | 7 | 7 | 6 | 6 | 7 | 7 |
| `industry_category` | 8 | 8 | 8 | 8 | 8 | 8 | 8 |
| 업력 | 10 | 10 | 10 | 10 | 10 | 9 | 9 |
| `replacement_avg_12m` | 9 | 9 | 9 | 9 | 9 | 10 | 10 |

**상위 10 의 구성원이 7배치 내내 동일하다.** 순서만 한두 칸 바뀐다.
11위 이하는 더 흔들리지만 M9 정의가 상위 10 이므로 기준 대상이 아니다.

**쌍별 지표** `[실측]`

| 쌍 | 상위10 교집합 | 순위 상관 | 최대 순위 이동 |
| --- | ---: | ---: | ---: |
| **① 25-12→26-01** | **10/10** | **0.903** | **3칸** |
| ② 26-01→26-02 | 10/10 | 0.988 | 1칸 |
| ③ 26-02→26-03 | 10/10 | 0.976 | 1칸 |
| ④ 26-03→26-04 | 10/10 | **1.000** | **0칸** |
| ⑤ 26-04→26-05 | 10/10 | 0.976 | 1칸 |
| ⑥ 26-05→26-06 | 10/10 | **1.000** | **0칸** |

**교집합은 6쌍 모두 10/10 이라 변별력이 없다.** 쓸 수 있는 것은 순위 상관과 최대 이동이다.

**① 은 여기서도 예외다** — §7.7.2 와 같은 패턴이다.

```
사유별 건수        25-12    26-01 이후
고지금액 변동      1,425     830~1,110     2위 → 4위
4대보험 체납이력     824    1,049~1,256    6위 → 3위
```

「4대보험 체납이력」은 **이력이 쌓여야 잡히는 사유**다. 초기 배치에서 적게 나오는 것이 자연스럽다. 같은 이유로 제외한다.

**정상 띠 — ① 제외 5쌍 기준**

```
상위10 교집합      10 / 10        ← 하나라도 빠지면 경고
순위 상관          0.97 이상
최대 순위 이동     1칸 이하
```

뜻: **「상위 10 사유의 구성원이 바뀌거나 두 칸 이상 움직이면 들여다본다」**
M9 를 *"SHAP 사유가 뒤집히면 모델·데이터 이상 신호"* 로 정의한 §7.2 의 취지에 맞는다.

> 산출 근거와 실행한 조회 목록: `.context/report/2026-09-19_20-06-34_산출-사유순위변동-기준선.md`

#### 7.7.7 🔴 두 기준은 방향이 반대다 — 화면에 함께 적을 것

**§7.7.3 과 §7.7.6 은 정반대로 읽어야 한다.** 같은 비교 화면에 나란히 뜨므로 설명 없이 두면 반드시 오해가 난다.

| | 큐 겹침 (§7.7.3) | 사유 순위 (§7.7.6) |
| --- | --- | --- |
| 모집단 | 63.9만 중 상위 3,000 | 사유 종류 16~19개 |
| 정상 범위 | 상위100 겹침 **19~31%** | 상위10 교집합 **10/10** |
| 성격 | 매우 불안정 | 거의 고정 |
| **읽는 법** | **「많이 바뀌는 것이 정상」** | **「바뀌면 이상」** |

설명이 없으면 이런 오해가 난다 — *"하나는 20%인데 정상이고 하나는 100%인데 경고?"*

**두 지표는 서로 보완한다.** 큐가 흔들려도 사유가 그대로면 모델이 같은 근거로 판단하고 있다는 뜻이다. **사유까지 흔들리면 그때가 진짜 이상이다.**
...
| ~~Q12 (승석)~~ | ~~기준선 띠(§7.7)를 어떤 지표·어떤 폭으로 정할지~~ → **2026-09-18 종결. §7.7.1~7.7.3 에 실측 띠 기재** |
| ~~Q13 (승석)~~ | ~~`reasons` 상위 사유의 순위 변동을 이상 신호로 볼 기준~~ → **2026-09-19 종결. §7.7.6 에 실측 기준 기재** |
