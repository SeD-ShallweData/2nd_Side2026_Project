...
> **Q4 보충** — 「값이 같은가」를 등호로 검사하면 **매 배치 98.9% 가 실패**한다.
> 자가 검증 도구는 허용오차 `1e-4` 로 비교하도록 구현돼 있다(`RISK_FULL_TOLERANCE`).
> 산출 측이 큐 파일에도 전체 정밀도를 넣어 주면 등호로 되돌릴 수 있다. **어느 쪽으로 갈지 결정이 필요하다.**
...
| 8 | 큐 파일 `risk_full` 정밀도 | 🔴 Q4 — 전체 정밀도를 넣을지, 허용오차 `1e-4` 를 계약으로 확정할지 |
