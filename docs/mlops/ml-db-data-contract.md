...
> **Q6 보충** — 이 값은 **업종 단위 사망률**이다. 임금체불 모델에 산업재해 성격의 값을 넣는 것이라
> 「임금체불과 산업재해를 합산하지 않는다」는 설계 원칙과 어떻게 정합할지 정리가 필요하다.
> 모델 내부 피처로만 쓰고 화면·상담에 노출하지 않는다면 원칙과 충돌하지 않는다.
>
...
| 2 | `industry_death_rate_2023` 추가 여부 | 🟡 Q6 — 산출 측 계획 대기 |
| 3 | `risk_calibrated` 채울 시점 | 🟡 Q7 — 산출 측 계획 대기. 채워지면 표현 정책 개정 필요 |
| 4 | 안정 `firm_id` 도입 | 🔴 Q8 — **「월 0.24%」 산출 방법 확인이 선행** |
| 5 | 산업재해 계약 문서 | 후속 (§12) |
| 7 | `risk_full` 의 이론적 범위 | 🟡 Q2 — 확정되면 자가 검증 V6 을 경고에서 **오류로 승격** |
