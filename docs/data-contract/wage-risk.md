...
강조하면 거의 모든 사업장이 경고로 보인다. 담당(수현·민서)과 협의가 필요하다.
...
> (`rank <= 100`). 위 표가 실측 기준이며, **주석 수정이 별도로 필요하다**(11.4절).
...
> SHAP 상위 3피처 한글명. ⚠️ **위험큐 상위 3000곳에만 존재한다.**
> 나머지 사업장에는 위험사유가 없으며 **지어내면 안 된다**.

실측 예시(상위 5건):

```
체납액, imputed_months_count, door1_maxmonths
imputed_months_count, 체납액, 고지금액 변동
```

**한글명과 영문 컬럼명이 섞여 있다.** `imputed_months_count`(결측 보정 개월수)가 상위 5건
전부에 등장한다. 감독관에게 그대로 노출할지 한글 라벨로 변환할지는 **미확정 — 별도 결정 필요**.

`shap_value` 원값은 POL-03에 따라 사용자·감독관 모두에게 노출하지 않는다. 노출하는 것은
피처 **이름**뿐이다.
...
둘 중 무엇을 표시할지는 화면별로 지정해야 한다 — **미확정 항목**(수현·민서와 협의).
...
코드를 뷰 기반으로 바꿀지는 별도 과제다(11.5절). 그 전까지 이 문서는 **현재 구현을 기술한다.**
...
- 담당: `product/src/**` — 공통 파일 규칙상 윤빈 검토 필요
...
| 코드를 뷰로 바꾼다 | `product/src/**` 변경. 공통 파일 규칙상 윤빈 검토 필요 |
...
| 항목 | 결정 필요 | 관련 담당 |
| --- | --- | --- |
| `유보` 85.7% 편중의 화면 처리 | 시각 강조 수준 | 수현·민서 |
| 화면별 기준일 표시(`as_of_date` vs `target_month`) | 화면마다 지정 | 수현·민서 |
| `reasons` 영문 피처명 라벨화 | 감독관 화면 표기 | 민서 |
| 미연결 3개 지표 데이터 소스 | 8절 4개 항목 | 승석(별도 과제) |
