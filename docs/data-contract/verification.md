...
그런데 **월 단위 계수기 3종이 전부 12에서 멈춘다** [전체 7배치 · 3,855,848행].

| 컬럼 | 실측 범위 | 13 등장 |
| --- | --- | :-: |
| `n_months` | 1 ~ **12** | 0건 |
| `zero_emp_months` | 0 ~ **12** | 0건 |
| `salary_drop_consecutive` | 0 ~ **12** | 0건 |
| `imputed_months_count` | 0 ~ 7 | — |

```
imputed_months_count = 0 인 441,815곳  →  전부 n_months = 12
n_months + imputed_months_count        →  12 (3,013,932) / 13 (510,794)
imputed_ratio 역산 분모                →  12
```

> **모순이 아니라 층위가 다르다.** 달력 구간은 13개 시점, 그 위에서 세는 개월 수는 12가 상한이다.
> 둘의 관계는 확인되지 않았다. **주석에 「13개월」만 단독으로 쓰면** 읽는 사람이
> `n_months` 를 조회했을 때 12까지만 보여 막힌다. 미확정 항목으로 올렸다 (`wage-risk.md` 11.6절).
