...
> **이후 변동** — 이 0건은 **1회차(2026-08-31) 시점 값**이다. `0009` 운영 적용(2026-09-06)
> 으로 `v_posts`·`v_comments` 주석 2건이 생겼고, `0012` 적용(2026-09-13) 으로 뷰 주석이
> 하나 더 들어갔다. 최신 현황은 **§15.8**(4회차)을 본다.
...
> **이후 변동** — 위 값은 4회차(2026-09-09) 시점이다. `0012` 병합·적용(2026-09-13) 으로
> 저장소 `COMMENT ON` 은 **52건**이 됐고 뷰 주석 1건이 운영 DB 에 함께 들어갔다.
> `scored_active` 51컬럼용 재적용 SQL 은 작성·전달됐고 아직 실행 전이다.
...
2. **BOM 이 붙으면 적재가 실패한다.** 에러 메시지가 `got "﻿a", expected "a"` 로 나와
   **눈으로는 같은 `a` 인데 다르다고 말한다.** 원인을 모르면 진단이 매우 어렵다.
   실제 제출물 3종에 전부 BOM 이 있으므로(4회차 후속 조사), **그대로 두면 다음 배치에서
   이 메시지와 함께 적재가 멈춘다.** → 17.3 에서 조치했다.

### 17.3 조치 — 적재가 BOM 을 벗기고 읽도록 고쳤다

BOM 을 없애는 쪽이 아니라 **적재가 받아들이는 쪽**으로 정했다. 산출 측이 BOM 을 붙이는 것은
`AGENT_GUIDE` 의 `utf-8-sig` 지시를 따르는 것이고, 감독관이 **Excel 로 여는 파일의 한글**을
지키려면 BOM 이 있는 편이 낫기 때문이다.

| 대상 | 변경 |
| --- | --- |
| `db/scripts/ingest.sh` | 적재 직전에 **BOM 만 벗긴 사본**을 만들어 `\copy` 가 그것을 읽는다. 원본 불변, `batches.source` 프로버넌스도 원본 `$OUT` 기준 유지 |
| `db/scripts/self_check.py` | CSV 를 `utf-8-sig` 로 읽어 **적재와 같은 동작**을 하게 했다 |
| 자가 검증 규격 F2 | 🔴 오류 → 🟡 **경고** (「BOM 이 있다」는 사실만 알림) |

**검증** [실측 2026-09-14]

```
① 바이트 정확성   tail -c +4 와 dd skip=3 의 sha256 일치 · 466,268 → 466,265 (정확히 3바이트)
② 서버 적재       실제 감독관_위험큐_full.csv (BOM) → ❌ 실패
                  같은 파일 BOM 제거본        → ✅ 3,000행 적재 성공
③ 원본 불변       전처리 실행 후에도 원본 3종 선두 바이트 efbbbf 유지
④ BOM 없는 경우   SRC="$OUT" 그대로 → 전처리 없음, 기존 동작과 동일
⑤ 자가 검증 도구  BOM 있는 원본 3종 그대로 → 오류 0 · 통과 18 · 경고 4 · 종료 코드 0
```

⑤ 가 핵심이다. 고치기 전에는 도구가 첫 컬럼명을 `﻿사업장명` 으로 읽어 **C2(컬럼 이름)가
거짓으로 실패**했다 — 적재는 되는데 도구는 반려하는 모순이 생긴다. 둘을 같은 방식으로
읽게 해 해소했다.
