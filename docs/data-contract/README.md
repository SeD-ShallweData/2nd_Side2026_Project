...
| `watch` | **안전 신호 미확인** ⚪ (2026-09-08 결정, 회색) | `유보` (85.7%) | `상위10%` |
| `review` | **우선 확인 필요** | `배제_*` 3종 | `상위1%` · `상위5%` |
| `unknown` | **분석 자료 부족** | `유보_정보부족` | 그 밖 |

> 🔴 **`watch` 의 「추가 확인 권장」은 부정확한 표기다.**
> `유보` 는 **위험을 발견한 것이 아니라 안전 신호를 확인하지 못한 것**이다
> (판정 규칙상 「나머지 전부」). 「추가 확인 권장」은 무언가 발견된 것처럼 읽힌다.
> 권장 대안은 **「안전 신호 미확인」·「판단 보류」**.
> 근거·실측·영향 범위는 [`wage-risk.md` 3.1.2절](wage-risk.md).
