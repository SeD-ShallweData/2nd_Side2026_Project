# 긍정 신호 공개 계약 — C2 구현 상태

2026-09-18 갱신. 개수·확인 항목 공개 구현은 [PR #67](https://github.com/SeD-ShallweData/2nd_Side2026_Project/pull/67)로 2026-09-15 병합됐다. 이 문서는 현재 구현의 응답 계약을 설명한다. 병합 사실을 별도 정책 합의의 증거로 간주하지 않으며, 9월 8일의 개수 비공개 결정은 변경 이력으로 남긴다.
...
2026-09-17에는 공개 API 5개 사업장과 상담 6회를 확인해 개수·항목 전달을 관측했다. 당시 답변 형식 문제도 함께 기록했으며, 전체 DB 완전성·모델 정확도·브라우저 화면 검증으로 확대 해석하지 않는다. [실측 보고서](../prompt/2026-09-17-positive-signals-live-check.md)를 참고한다. 이후 상담 경로와 출력 처리는 [PR #96](https://github.com/SeD-ShallweData/2nd_Side2026_Project/pull/96)에서 변경됐으므로 당시 실패를 현재 미해결이라고 단정하지 않는다.
