 ...
 *  3. `firm_id` 는 **불변이 아니다.** 사업장명이 바뀌면 달라진다.
 *     그래서 `firms` 에 원본 `name`·`biz_no` 를 보존해 나중에 재매핑할 수 있게 둔다.
 *     ML 팀이 안정 firm_id 를 export 에 넣으면 그걸로 교체한다.
 *     「월 약 0.24%」는 폐업·회전(월 2.9~4.3%)이 아니라 **지속 사업장의 개명률**이다.
 *        이름-독립 키로 같은 사업장 확정 후 정규화 상호 변경률 — ML 측정 1,253/523,021=0.24%
 *        (2026-03→04 단일쌍), 별도 실측(개명분)도 약 0.19%/월. 단 단일쌍 점추정이라 변동 가능.
