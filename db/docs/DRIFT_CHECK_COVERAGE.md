...
> **한 줄 요약**
> `npm run check:migration-drift` 는 이제 **0006~0014 를 모두 검사한다.**
...
| `0014_real_hitman` | ✓ | **11개 키** — user_favorite_firms 테이블·컬럼·PK·FK(cascade)·인덱스 존재 확인 |
...
## 2026-09-19 추가 — 0014 등록

0014(`user_favorite_firms` 즐겨찾기 테이블)를 만들면서 이번에는 "나중에 공백을 메우려면"
...
4. `cd db && npm run test:migration-drift` — 12개 테스트 전부 통과. 그중
   ...
   `journal의 모든 migration이 등록 여부를 명시적으로 갖는다` 가 0014 키 누락·
   불일치를 잡는 안전망 역할을 했다.
