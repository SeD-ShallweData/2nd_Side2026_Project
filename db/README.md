...
`firm_id` 는 **불변이 아니다** — 사업장명이 바뀌면 달라진다.
그래서 `firms` 에 원본 `name`·`biz_no` 를 보존한다. ML 팀이 안정 ID 를 export 에 넣으면 교체한다.

> **변경 비율 「월 약 0.24%」의 의미** — 폐업·신설 회전(월 2.9~4.3%)이 아니라
> **지속 사업장의 개명률**이다. 이름-독립 키(사업자번호+적용일자+법정동·업종코드)로 같은
> 사업장을 먼저 확정한 뒤 그 안에서 정규화 상호가 바뀐 비율로, ML 산출 측 측정값
> **1,253/523,021 = 0.24%**(2026-03→04 단일쌍)이며 별도 실측(개명분만)에서도 **약 0.19%/월**로
> 같은 자릿수다. **단 단일 배치쌍 점추정이라 배치마다 흔들릴 수 있다.**
> 측정법·재현은 `docs/data-contract/wage-risk.md` §7.1.
