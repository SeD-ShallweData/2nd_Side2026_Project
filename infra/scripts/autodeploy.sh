...
# 못하도록 단언한다.
#
# 2026-09-13 변경: 09-12 → 09-30.
# 원래는 09-23 프리즈 후 무장하고 09-28(심사 전날)에 해제하는 일정이었다.
# 운영자 결정으로 **시연 준비 기간 내내 자동 배포를 쓰기로** 바꿨다.
# 09-30 은 발표 전날이므로, 발표 당일 첫 폴링에서 스스로 꺼지고 알림이 간다.
#
# ⚠️ 심사는 09-29~10-01 이다. 09-29·09-30 이틀은 심사와 겹친다 —
#    그 동안 무인 배포가 곤란하면 사람이 `--disarm` 으로 미리 끈다.
ARM_EXPIRES='2026-09-30'
