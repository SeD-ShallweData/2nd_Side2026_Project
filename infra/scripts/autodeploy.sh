...
#
# **발표일(2026-10-01)에는 절대 살아 있으면 안 된다.** 발표 중에 누가 오타 하나
# 고쳐 머지하면 그 순간 서비스가 끊긴다. CI 테스트가 이 값이 그 경계를 넘지
# 못하도록 단언한다.
#
# 2026-09-13 운영자 최종 확정: **2026-09-29 24:00 KST 까지.**
# (09-12 → 09-30 으로 올렸다가, 발표 직전 이틀이 급변하는 것이 부담이라 하루 당김)
#
# 이 값은 **날짜**이고 판정은 is_expired() 의 `>` 비교다. 즉 그 날 하루는
# 통째로 무장 상태이고 **다음 날 00:00 부터 만료**다 — "09-29 24:00 까지" 와
# 정확히 같은 뜻이다. 시각이 아니라 날짜로 비교하는 이유는, 자정을 넘긴 폴링이
# 경계에서 흔들리지 않게 하기 위해서다. 위의 export TZ='Asia/Seoul' 덕분에
# 이 비교는 언제나 한국 시간 기준이다.
#
# 실제 마지막 자동 배포 기회는 시간창 때문에 **09-29 22:30 KST** 다.
#
# ⚠️ 심사는 09-29~10-01 이다. 첫날(09-29)은 여전히 겹친다 —
#    그날 무인 배포가 곤란하면 사람이 `--disarm` 으로 미리 끈다.
ARM_EXPIRES='2026-09-29'
