...
기록 시점: 2026-09-12 · **갱신 2026-09-13**(만료일 09-12 → 09-30) · 대상 `infra/scripts/autodeploy.sh`,
...
**`ARM_EXPIRES = 2026-09-30`** (2026-09-13 변경, 이전 값 `2026-09-12`)

원래 설계는 `09-23` 프리즈 이후 무장하고 `09-28`(심사 전날)에 자동 해제하는
것이었다. 운영자 결정으로 **시연 준비 기간 내내 자동 배포를 쓰기로** 바꿨다 —
main 에 PR 이 병합되면 GCP 서버가 스스로 따라온다.

`09-30` 을 고른 이유는 **발표 전날**이기 때문이다. 발표 당일(`10-01`) 첫 폴링에서
폴러가 스스로 무장을 해제하고 한 번 알린다. 사람이 잊어도 발표 중에는 꺼져 있다.

> ⚠️ **심사는 `09-29~10-01` 이다.** 09-29·09-30 이틀은 자동 배포와 겹친다.
> 알고 한 선택이지만, 그 동안 무인 배포가 곤란하면 **심사 시작 전에 사람이 끈다.**
>
> ```bash
> sudo moneyworry-autodeploy --disarm
> ```
>
> CI 상한(`LATEST_ALLOWED_EXPIRY`)이 지키는 것은 이제 **발표 당일 하나**다.
> 그건 어떤 이유로도 넘기지 않는다.

| 날짜 | 자동 배포 | 비고 |
| --- | :---: | --- |
| ~09-30 | ✅ 무장 가능 | main 병합 → 10분 안에 서버 반영 (08:00–22:30 KST) |
| 09-29~09-30 | ⚠️ 심사와 겹침 | 곤란하면 `--disarm` |
| **10-01 (발표)** | ❌ **스스로 꺼진다** | 첫 폴링에서 자동 해제 + Discord 알림 |
