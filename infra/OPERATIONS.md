...
로그인 5회 실패에 따른 15분 잠금은 웹 프로세스 메모리에만 저장한다. 현재 단일 VM·단일
`moneyworry-web` 프로세스에서는 모든 로그인 요청이 같은 상태를 공유하지만, 다음 경우에는 실패 횟수와
남은 잠금 시간이 초기화된다.

- 코드 배포, 수동 재시작, 장애 또는 health-watch에 따른 웹 프로세스 재시작
- VM 재부팅과 `daily18h` 예약 종료·시작

이 상태는 백업·복원 대상이 아니며 여러 프로세스나 VM 사이에 공유되지 않는다. 재시작 이후에도 잠금을
보장하거나 웹을 수평 확장하기 전에는 DB 또는 Redis 같은 공유 저장소로 이전한다.
